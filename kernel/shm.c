...
long sys_shm_at(int key, unsigned long base_addr, unsigned int flag)
{
	printk("shm_at\n");
	return shm_at(key, base_addr, flag);
}

long sys_shm_dt(int key, unsigned long base_addr)
{
	return shm_dt(key, base_addr);
}

long sys_shm_free(int key)
{
	return shm_free(key);
}

long sys_shm_get(int key, unsigned long size)
{
