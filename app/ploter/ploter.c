	...
	for(;recv( -1, &msg, 1);)
	{
		...
		switch( msg.type&(~MSG_TYPE_BLOCK) )
		{
			...
			case UM_MOUSEACT:
				...
				btn = msg.arg4;
				char buf[100];
				sprintf(buf, "(%4d,%4d) %s %s %s", x, y, 
					btn&1?"L":" ", btn&2?"R":" ", btn&4?"M":" ");
				draw_string(0, 0, buf, 0);
				w_refresh(hwnd, buff, 0, 0, 600, 15);
				break;
			case UM_EXIT:
				if(w_destroy(hwnd))
					wcount--;
...
int main()
{
	...
	hwnd = w_create(0, 0, 0, 600, 600, "abcdef");
	mhwnd = hwnd;
	...
	for(x = 0; x < 600; x++)
	{
		...
		{
			double xx,yy;
			xx = ((double)x) * 3 * 3.14159 / 600;
			yy = ((double)y) * 3 * 3.14159 / 600;
			z=sin(xx+xx+yy+yy)*sin(2*xx*yy);
				buff[y*info.x + x] = RGB(0,(int)(62 * (z + 1.01)), 0);
		}
	}
	w_refresh(mhwnd, buff,0 ,0 , 600, 600);
